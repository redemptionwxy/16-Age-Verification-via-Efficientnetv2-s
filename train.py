...
Key refinements (v3):
  - Leak-aware asymmetric loss: predicting 25 for a 14-year-old costs 5× more
    than predicting 5. Directly targets under-16 leak rate.
  - EfficientNetV2-S backbone (default): stronger features, still BSD-licensed.
...
Usage:
    ...
    python train.py --dataset combined \\
        ...
        --utkface-root Datasets/UTKFace/UTKFace \\
        --leak-penalty 5.0 --age-weight 2.0 --balance \\
        --exclude-csv bad_labels.csv --name combined_v3
...
# ---------------------------------------------------------------------------
# Leak-aware asymmetric loss — directly targets under-16 leak rate
# ---------------------------------------------------------------------------
def leak_aware_loss(pred, target, boundary=16, leak_penalty=5.0, age_weight=2.0):
    """
    Asymmetric L1 loss designed to minimise under-16 leak:

    1. LEAK PENALTY: if true_age < boundary AND pred >= boundary,
       the error is multiplied by `leak_penalty` (default 5×).
       → The model learns "never overestimate an under-16 person."

    2. AGE WEIGHT: errors near the boundary (within ±focus_width) are
       weighted up to `age_weight` via a Gaussian centred at boundary.
       → Focuses model capacity on the decision-critical age range.

    With leak_penalty=1.0 and age_weight=1.0 this reduces to plain L1.
    """
    base_loss = torch.abs(pred - target)
    t = target.detach()

    # --- leak mask: under-16 predicted as adult -------------------------
    leaked = (t < boundary) & (pred.detach() >= boundary)
    leak_mult = torch.where(leaked, leak_penalty, 1.0)

    # --- age weighting: extra focus near the boundary -------------------
    if age_weight > 1.0:
        sigma = 10.0                                          # width of focus
        age_mult = 1.0 + (age_weight - 1.0) * torch.exp(
            -((t - boundary) ** 2) / (2 * sigma ** 2))
    else:
        age_mult = 1.0

    return (base_loss * leak_mult * age_mult).mean()
...
def run(args):
    ...
    print(f"  loss      : leak-aware L1 (leak_penalty={args.leak_penalty}, "
              f"age_weight={args.age_weight})")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--backbone", default="efficientnetv2-s",
    ...
    # Refinement flags
    ap.add_argument("--leak-penalty", type=float, default=5.0,
                    help="Penalty multiplier when under-16 is predicted as "
                         "adult. 5.0 = 5x penalty. 1.0 = symmetric L1. "
                         "Default: 5.0.")
    ap.add_argument("--age-weight", type=float, default=2.0,
                    help="Extra weight for errors near the boundary "
                         "(Gaussian centred at 16). 1.0 = uniform. "
                         "Default: 2.0.")
